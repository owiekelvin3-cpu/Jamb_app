  ...
  return (
    <div className="app-shell">
      <header className="hero-panel">
        <div>
          <div className="app-badge">{APP_TITLE}</div>
          <h1>Start from zero and build to 300+ in JAMB.</h1>
          <p className="hero-copy">
            A complete beginner-friendly school system with 4 levels for each subject, locked progression, video support, practice and strict exam preparation.
          </p>
          <div className="hero-actions">
            <button className="primary-button" onClick={navigateToDashboard}>
              Go to dashboard
            </button>
            {mockUnlocked && (
              <button className="secondary-button" onClick={launchMockExam}>
                Start Mock Exam
              </button>
            )}
          </div>
          ...
          {topicTab === 'test' && (
            <div className="topic-card">
              ...
              <div className="topic-actions">
                <button className="primary-button" onClick={submitTopicTest}>
                  Submit test
                </button>
                <button className="tertiary-button" onClick={resetTopicReview}>
                  Reset answers
                </button>
              </div>
              {topicResult && (
                <div className={`topic-result ${topicResult.passed ? 'passed' : 'failed'}`}>
                  <h4>{topicResult.passed ? 'Passed!' : 'Needs review'}</h4>
                  <p>Score: {formatScore(topicResult.score)}</p>
                  {topicResult.missed.length > 0 && (
                    <div>
                      <p>Review the missed questions below:</p>
                      <ul>
                        {topicResult.missed.map((missed, index) => (
                          <li key={index}>{missed}</li>
                        ))}
                      </ul>
                    </div>
                  )}
                  {!topicResult.passed && <p>Focus on the lesson, practice again and retake the test.</p>}
                  {topicResult.passed && <p>Great job! You can move to the next level when ready.</p>}
                </div>
              )}
            </div>
          )}
        ...
        <section className="mock-panel">
          <div className="section-top">
            <div>
              <button className="link-button" onClick={navigateToDashboard}>&larr; Back to dashboard</button>
              <h2>Mock Exam</h2>
              <p>Full JAMB simulation timed for 2 hours. Unlocks only after completing all Level 4 topics.</p>
            </div>
            <div className="topic-status-card" style={{ borderColor: mockUnlocked ? "#0f766e" : "#9ca3af" }}>
              <span>Mock exam status</span>
              <strong>{mockUnlocked ? "Ready" : "Locked"}</strong>
            </div>
          </div>

          {!mockUnlocked && (
            <div className="topic-card">
              <p>The mock exam unlocks after you complete Level 4 in every subject.</p>
              <div className="info-grid">
                {CURRICULUM.map((subject) => {
                  const subjectCompleted = subject.levels[subject.levels.length - 1].topics.every(
                    (topic) => progress[subject.id][subject.levels[subject.levels.length - 1].id][topic.id]?.passed
                  );
                  return (
                    <div key={subject.id} className="status-card">
                      <strong>{subject.name}</strong>
                      <span>{subjectCompleted ? "Level 4 complete" : "Complete Level 4"}</span>
                    </div>
                  );
                })}
              </div>
          ...
          {mockUnlocked && !mockResult && (
            <div className="topic-card">
              <div className="mock-header">
                <p>Time remaining: <strong>{formatTime(mockSeconds)}</strong></p>
                <p>Questions: {MOCK_EXAM.length}</p>
              </div>
              {mockActive ? (
                <ol className="question-list">
                  {MOCK_EXAM.map((question, index) => (
                    <li key={index} className="question-item">
                      <p>{question.q}</p>
                      <div className="options-grid">
                        {question.options.map((option, optionIndex) => (
                          <button
                            key={optionIndex}
                            className={`option-button ${mockAnswers[index] === optionIndex ? 'selected' : ''}`}
                            onClick={() => setMockAnswers((current) => ({ ...current, [index]: optionIndex }))}
                          >
                            {option}
                          </button>
                        ))}
                ...
                <p>Press Start to begin the mock exam.</p>
              )}
              <div className="topic-actions">
                {!mockActive && (
                  <button className="primary-button" onClick={launchMockExam}>
                    Start exam
                  </button>
                )}
                {mockActive && (
                  <button className="primary-button" onClick={submitMockExam}>
                    Submit exam
                  </button>
                )}
              </div>
          ...
          {mockResult && (
            <div className="topic-card">
              ...
              <button className="secondary-button" onClick={navigateToDashboard}>
                Return to dashboard
              </button>
            </div>
          )}
